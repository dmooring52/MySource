			...
			if (IsValid())
			{
				...
				foreach (TSCVenues venue in gd_tscvenues)
				{
					if (venue.state == 'I')
					{
						...
						string result = _scc.GeneralStore("TSCVenues", "INSERT", xml);
					...
					{
						string xml = venue.GetDataXml();
						string result = _scc.GeneralStore("TSCVenues", "UPDATE", xml);
						if (!(result != null && IsNumber(result) && int.Parse(result) > 0))
						{
							MessageBox.Show("Save error: " + venue.VenueKey + " - " + result);
						}
				...
				foreach (TSCVenues venue in db_tscvenues)
				{
					...
					{
						string xml = venue.GetDataXml();
						string result = _scc.GeneralStore("TSCVenues", "DELETE", xml);
						if (!(result != null && IsNumber(result) && int.Parse(result) > 0))
						{
				...
				if (recordsadded > 0 || recordschanged > 0 || recordsremoved > 0)
				{
					db_tscvenues.Clear();
					gd_tscvenues.Clear();
					string xml = _scc.GeneralStore("TSCVenues", "GET", (new TSCVenues()).GetDataXml());
					LoadTSCVenues(xml);
					RefreshGrid();
					if (_queuerForm != null)
