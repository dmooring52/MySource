	...
	public partial class EventsControl : UserControl
	{
		...
		private void EventsControl_Load(object sender, EventArgs e)
		{
			...
			if (_queuerForm != null)
			{
				...
				LoadTSCEvents(_queuerForm.controlTag.eventsXml);
			}
			else
		...
		public void LoadVenues()
		{
			...
			try
			{
				XmlNodeList nodelist = _queuerForm.controlTag.venuesXml.SelectNodes("/Root/Data");
			...
			if (IsValid())
			{
				...
				foreach (TSCEvents tscevent in gd_tscevents)
				{
					if (tscevent.state == 'I')
					{
						...
						string result = _scc.GeneralStore("TSCEvents", _queuerForm.controlTag.eventsXml, "INSERT", xml);
					...
					{
						string xml = tscevent.GetDataXml();
						string result = _scc.GeneralStore("TSCEvents", _queuerForm.controlTag.eventsXml, "UPDATE", xml);
						if (!(result != null && IsNumber(result) && int.Parse(result) > 0))
						{
							MessageBox.Show("Save error: " + tscevent.EventKey + " - " + result);
						}
				...
				foreach (TSCEvents tscevent in db_tscevents)
				{
					...
					{
						string xml = tscevent.GetDataXml();
						string result = _scc.GeneralStore("TSCEvents", _queuerForm.controlTag.eventsXml, "DELETE", xml);
						if (!(result != null && IsNumber(result) && int.Parse(result) > 0))
						{
				...
				if (recordsadded > 0 || recordschanged > 0 || recordsremoved > 0)
				{
					...
					if (_queuerForm != null)
					{
						_queuerForm.controlTag.venuesXml = _scc.GetTable("TSCEvents", "");
						LoadTSCEvents(_queuerForm.controlTag.venuesXml);
						RefreshGrid();
