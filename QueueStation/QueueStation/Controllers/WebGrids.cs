...
namespace QueueStation.Controllers
{
    ...
    public class SongsGrid
    {
        ...
        public bool bydisk { get { return _bydisk; } set { _bydisk = value; } }
        ...
        public object GetNextPage(object o)
        {
            return o;
        }
    ...
    public class SingersSongsGrid
    {
        private string _command = "";
        private WebGrid _grid = null;
        private List<MVCSingersSongs> _singers = new List<MVCSingersSongs>();
        private List<MVCSingersSongs> _singersref = new List<MVCSingersSongs>();
        public string command { get { return _command; } set { _command = value; } }
        public WebGrid grid { get { return _grid; } set { _grid = value; } }
        public List<MVCSingersSongs> singers { get { return _singers; } set { _singers = value; } }
        public List<MVCSingersSongs> singersref { get { return _singersref; } set { _singersref = value; } }

        /*
        public string SingerData(string singerkey, string column, string data)
        {
            MVCSingersSongs target = null;
            MVCSingersSongs targetreference = null;
            foreach (MVCSingersSongs q in singers)
            {
                if (q.SingerKey == singerkey)
                {
                    target = q;
            ...
            foreach (MVCSingersSongs q in singersref)
            {
                if (q.SingerKey == singerkey)
                {
                    targetreference = q;
        ...
        */
        ...
        public string Save()
        {
            ...
            {
                List<MVCSingersSongs> tobeadded = new List<MVCSingersSongs>();
                foreach (MVCSingersSongs q in singers)
                {
                    bool found = false;
                    foreach (MVCSingersSongs qref in singersref)
                    {
                        if (q.KeyEquals(qref) == true)
                        {
                            found = true;
                            if (MVCSingersSongs.FieldsEqual(q, qref) == false)
                            {
                                result = client.GeneralStore("TSCSingersSongs", "UPDATE", q.GetDataXml());
                                if (result != null && Utility.IsNumber(result) && int.Parse(result) > 0)
                                {
                                    recordschanged++;
                                }
                ...
                if (tobeadded.Count > 0)
                {
                    foreach (MVCSingersSongs tba in tobeadded)
                    {
                        result = client.GeneralStore("TSCSingersSongs", "INSERT", tba.GetDataXml());
                        if (result != null && Utility.IsNumber(result) && int.Parse(result) > 0)
                        {
                            recordschanged++;
                        }
